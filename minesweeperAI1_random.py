    ...
    def performAI(self, boardState):
        ...
        if len(list(bombsFoundSoFar)) == self.numBombs:
            ...
            for f in frontier:
                ...
                if f[2] - count_visible_bomb == count_unopened_neighbors:
            ...
            if is_certain == False:
                ...
                for f in frontier:
                    ...
                    neighbors = self.get_uopened_neighbors(f[0], f[1], boardState)
                    count_unopened_neighbors = len(neighbors)
                    count_known_bombs_around = self.get_num_of_bombs_around(f[0], f[1], bombsFoundSoFar)
                    diff = count_unopened_neighbors - (f[2] - count_known_bombs_around)
                    #print(diff)
                    if diff > max_diff:
                        max_diff = diff
                        min_prob_frontier = f
                squareToOpen = random.choice(self.get_uopened_neighbors(min_prob_frontier[0], min_prob_frontier[1], boardState))
